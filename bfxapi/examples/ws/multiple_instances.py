...
class Routine:
    ...
    async def do(self):
        ...
        to_sub = [
            f"t{ticker}"
            for ticker in get_random_list_of_tickers()
            if f"t{ticker}" not in subbed_tickers
        ]
        ...
        to_unsub = [
            f"t{ticker}"
            for ticker in subbed_tickers
            if f"t{ticker}" in get_random_list_of_tickers()
        ]
        if len(to_unsub) > 0:
...
async def on_subscribe(instance, subscription):
    ...
    instance.subscriptions[subscription.channel_name][
        subscription.symbol
    ] = subscription.chan_id
