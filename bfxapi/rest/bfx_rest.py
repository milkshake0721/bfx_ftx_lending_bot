...
class BfxRest:
    ...
    async def post(self, endpoint, data={}, params=""):
        ...
        headers = generate_auth_headers(self.API_KEY, self.API_SECRET, endpoint, sData)
        ...
        async with aiohttp.ClientSession() as session:
            async with session.post(url + params, headers=headers, data=sData) as resp:
                ...
                if resp.status < 200 or resp.status > 299:
                    raise Exception(
                        "POST {} failed with status {} - {}".format(
                            url, resp.status, text
                        )
                    )
                parsed = json.loads(text, parse_float=self.parse_float)
    ...
    async def get_seed_candles(
        self, symbol, tf="1m", start=None, end=None, limit=10000, sort=0
    ):
        """
    ...
    async def get_public_candles(
        self, symbol, start, end, section="hist", tf="1m", limit=100, sort=-1
    ):
        """
    ...
    async def get_public_trades(self, symbol, start, end, limit=100, sort=-1):
        ...
        return sorted(
            trades, key=lambda x: (x[1], x[0]), reverse=True if sort == 1 else False
        )
    ...
    async def get_market_average_price(
        self, symbol, amount=None, period=None, rate_limit=None
    ):
        """
    ...
    async def get_margin_info(self, symbol="base"):
        ...
        if symbol == "base":
            ...
            return [
                MarginInfo.from_raw_margin_info(record) for record in raw_margin_info
            ]
        else:
    ...
    async def get_trades(self, start, end, symbol=None, limit=25):
        ...
        """
        endpoint = (
            "auth/r/trades/{}/hist".format(symbol) if symbol else "auth/r/trades/hist"
        )
    ...
    async def get_funding_info(self, symbol):
        endpoint = "auth/r/info/funding/{}".format(symbol)
        info = await self.post(endpoint)
        return [FundingInfo.from_raw_info(info)]
    ...
    async def get_ledgers(self, symbol, start, end, limit=25, category=None):
        ...
        """
        endpoint = (
            "auth/r/ledgers/{}/hist".format(symbol) if symbol else "auth/r/ledgers/hist"
        )
        params = "?start={}&end={}&limit={}".format(start, end, limit)
        if category:
    ...
    async def submit_wallet_transfer(
        self, from_wallet, to_wallet, from_currency, to_currency, amount
    ):
        """
    ...
    async def submit_update_order(
        ...
        if leverage != None:
            payload["lev"] = str(leverage)
        flags = calculate_order_flags(hidden, close, reduce_only, post_only, False)
        payload["flags"] = flags
    ...
    async def claim_position(self, position_id, amount):
        ...
        """
        payload = {"id": position_id, "amount": f"{amount * -1}"}
    ...
    async def set_alert(self, type, symbol, price):
        ...
        payload = {"type": type, "symbol": symbol, "price": price}
    ...
    async def calc_order_avail(self, symbol, type, lev, dir=None, rate=None):
        ...
        payload = {"symbol": symbol, "type": type, "lev": lev}
    ...
    async def submit_pulse_comment(
        self,
        title,
        content,
        parent,
    ...
    async def generate_invoice(self, amount, wallet="exchange", currency="LNX"):
        ...
        payload = {"wallet": wallet, "currency": currency, "amount": amount}
        message = await self.post(endpoint, payload)
        return message
    ...
    async def submit_invoice(
        ...
        webhook,
        redirect_url,
        customer_info_nationality,
        customer_info_resid_country,
        customer_info_resid_city,
        customer_info_resid_zip_code,
        customer_info_resid_street,
        customer_info_full_name,
        customer_info_email,
        customer_info_resid_state=None,
        customer_info_resid_building_no=None,
        duration=None,
    ):
        """
