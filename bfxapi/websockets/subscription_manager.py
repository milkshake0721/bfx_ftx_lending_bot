...
class SubscriptionManager:
    ...
    async def unsubscribe_all(self):
        ...
        for chan_id in self.subscriptions_chanid:
            ...
            if sub.is_subscribed():
                task_batch += [asyncio.ensure_future(self.unsubscribe(chan_id))]
        if len(task_batch) == 0:
            return
    ...
    async def resubscribe_by_socket(self, socket_id):
        ...
        for sub in self.subscriptions_chanid.values():
            if sub.socket.id == socket_id:
                task_batch += [asyncio.ensure_future(self.resubscribe(sub.chan_id))]
        if len(task_batch) == 0:
            return
        await asyncio.wait(*[task_batch])
    ...
    async def resubscribe_all(self):
        ...
        task_batch = []
        for chan_id in self.subscriptions_chanid:
            task_batch += [asyncio.ensure_future(self.resubscribe(chan_id))]
        if len(task_batch) == 0:
            return
        await asyncio.wait(*[task_batch])
