import pytest
from .helpers import (
    create_stubbed_client,
    ws_publish_connection_init,
...
async def test_checksum_generation():
    ...
    await client.ws.publish(
        """[123, [[0.0000886,1,1060.55466114],[0.00008859,1,1000],[0.00008858,1,2713.47159343],[0.00008857,1,4276.92870916],[0.00008856,2,6764.75562319],
    ...
    await client.ws.publish(
        "[{},[0.00008837,1,56.54876269]]".format(chanId), is_json=False
    )
    await client.ws.publish(
...
async def test_checksum_really_samll_numbers_generation():
    ...
    ## send orderbook snapshot
    await client.ws.publish(
        """[123, [[0.00000121,5,249013.0209708],[0.0000012,6,518315.33310128],[0.00000119,4,566200.89],[0.00000118,2,260000],[0.00000117,1,100000],
    ...
    [0.00000186,1,-4000],[0.00000189,1,-10000.686624],[0.00000191,1,-14500]]]""",
        is_json=False,
    )
    ## send some more price updates
    await client.ws.publish(
        "[{},[0.00000121,4,228442.6609708]]".format(chanId), is_json=False
    )
    await client.ws.publish(
