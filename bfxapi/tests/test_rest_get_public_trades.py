...
async def run():
    ...
    candles = await bfx.rest.get_public_candles(
        start=start, symbol="tBTCUSD", end=None, tf="1h", sort=1, limit=1
    )
    candle = candles[0]
    price = candle[1]
    ...
    orders_ids = []
    trades = await bfx.rest.get_public_trades(
        start=1617519600000, limit=5, symbol="tBTCUSD", end=None, sort=1
    )
    print(trades)
    for trade in trades:
    ...
    timestamps = []
    trades = await bfx.rest.get_public_trades(
        start=1617519600000, limit=5000, symbol="tLEOUSD", end=None, sort=1
    )
    print(trades)
    for trade in trades:
        orders_ids.append(trade[0])
